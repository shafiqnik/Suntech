"""
Test script to verify BLE beacon parsing with the provided message
"""
import sys
from suntech_parser import SuntechParser

# The BLE message provided by the user
test_message_hex = ''

def test_ble_parsing():
    """Test parsing of the BLE message"""
    print("=" * 80)
    print("Testing BLE Message Parsing")
    print("=" * 80)
    ...
    try:
        ...
        if len(sensors) == 0:
            ...
            # Display other beacons
            if other_beacons:
                print(f"Other BLE Devices: {len(other_beacons)}")
                print("-" * 80)
                for beacon in other_beacons:
                    print(f"  {beacon['index']}. {beacon['mac']}")
                    print(f"     RSSI: {beacon['rssi']} dBm ({beacon['rssi_hex']})")
                    print()
        ...
        if raw_data:
            ...
            for pattern in patterns:
                pos = raw_upper.find(pattern)
                if pos >= 0:
                    print(f"  ✓ Found '{pattern}' at hex position {pos} (byte {pos // 2})")
                    # Show context
                    start = max(0, pos - 20)
                    end = min(len(raw_upper), pos + 30)
